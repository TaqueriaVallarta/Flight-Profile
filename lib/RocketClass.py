...
class Rocket:
    def __init__(self, drag_setup: DragSetup, dry_mass, time=0,
                 dt=.05, initial_height_msl=0.0):
        self.fudge_factor = .8
        self.burn_time = 20
        self.drag_setup = drag_setup  # drag setup class
        self.motor = Motor(dry_mass, self.burn_time)  # motor class
        ...
        self.velocity = 0
        self.tank_thickness = .25 * 2.52/100 # Thickness of tank in meters
        self.tank_material_density = 2700 # density of t6-6061 from mat web in kg/m^3
        ...
        self.dataframe = DataFrame(
            columns=['Time', 'Height AGL', 'Height MSL', 'Velocity',
                     ...
                     'Cross-sectional Area', 'Flight State', 'Weight', 'Total Force', 'OF Ratio',
                     'Total Mass Flow'])
        self.dataframe: DataFrame
        self.values = {
            ...
            "exit_pressure": self.motor.exit_pressure
        }
    ...
    def set_vals_to_vars(self):
        ...
        self.values["exit_pressure"] = self.motor.exit_pressure
        self.values["burn_time"] = self.motor.burn_time
        self.motor.other_mass = self.dry_mass
        self.burn_time = self.motor.burn_time
    ...
    def set_vars_to_new(self, new_values: dict):
        ...
        self.motor.exit_pressure = new_values.get("exit_pressure", self.motor.exit_pressure)
        self.motor.burn_time = new_values.get("burn_time", self.motor.burn_time)
        self.motor.other_mass = self.dry_mass
        self.burn_time = self.motor.burn_time
        self.motor.update_self()
        self.drag_setup.main_parachute.start_surface_area = self.drag_setup.reefed_parachute.surface_area
    ...
    def acceleration(self, height_msl, velocity, time):
        drag, self.dt = self.drag_setup.calculate_drag_force(velocity, height_msl, self.time, self.burn_time)
        thrust = self.motor.thrust(time, self.drag_setup.atmosphere.pressure(height_msl), self.fudge_factor)
    ...
    def specific_impulse(self):
        thrust = self.motor.thrust(self.time,self.drag_setup.atmosphere.pressure(self.height_msl), self.fudge_factor)
        mass_flow = self.motor.mass_flow(self.time)
        g = self.drag_setup.atmosphere.g
        return (thrust/mass_flow/g).magnitude
    ...
    def output(self):
        return [self.time, self.height_agl, self.height_msl, self.velocity,
                ...
                self.mass(self.time), self.motor.thrust(self.time, self.drag_setup.atmosphere.pressure(self.height_msl), self.fudge_factor),
                self.drag_setup.calculate_drag_force(self.velocity, self.height_msl, self.time, self.burn_time)[0],
                ...
                self.acceleration(self.height_msl, self.velocity, self.time) * self.mass(self.time), self.motor.OF_t(self.time).magnitude,
                self.motor.mass_flow(self.time).magnitude]

    def flight_state(self):
        if self.time <= self.motor.burn_time.magnitude:
    ...
    def sim_to_apogee(self):
        ...
        return self.height_agl, self.dataframe['Acceleration'][0]
